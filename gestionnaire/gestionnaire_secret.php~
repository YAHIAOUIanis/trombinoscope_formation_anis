...
<body>
	...
	<section class="formProf">
		<form class="formArbo" method="post" action="gestionnaire_secret.php">
			<h2 style="text-align:center">Gestion des Secrétaires</h2>
		...
		<?php
			if(isset($_POST['id']) && isset($_POST['nom']) && isset($_POST['prenom']) && isset($_POST['mdp'])){
				//cette partie ajoute les secrétaires
				if(isset($_POST['ajouter'])){
					...
					if(!existeLineCSV("./dataSecret.csv",$listData)){
						...
						echo"<p style='text-align:center; color:green;'>Secrétaire ajouté avec succès!</p>\n";
					}else{
						echo"<p style='text-align:center; color:red;'>Ce Secrétaire est déjà inscrit!</p>\n";
					}	
				}
				//cette partie supprime les secrétaires

				if(isset($_POST['supprimer'])){
					$listWords = array($_POST['nom'],$_POST['prenom'],$_POST['id']);
					$remove = deleteLineText("./dataSecret.csv",$listWords);
					if($remove){
						echo"<p style='text-align:center; color:green;'>Secritaire supprimé avec succès!</p>\n";
					}
					else {
						echo"<p style='text-align:center; color:red;'>Secrétaire inexistant!</p>\n";
					}	
				}
			}
		?>		
	...
	<section class="liste_profs">
		<h2 style="text-align:center;">Liste des Secrétaires </h2>
		<?php
